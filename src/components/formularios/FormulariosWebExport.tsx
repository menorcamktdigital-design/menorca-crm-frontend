...
import { useTodosFormulariosWeb, type FiltrosWeb } from "@/hooks/useFormulariosWeb";
...
// Igual que FormulariosExport (Meta) pero sobre /formularios/web: al hacer
// clic se piden todos los leads filtrados, paginando hasta el final.
export default function FormulariosWebExport({
  filtros,
  total,
}: {
  filtros: FiltrosWeb;
  ...
  const { refetch } = useTodosFormulariosWeb(filtros, false);
  ...
  const exportar = async () => {
    ...
    try {
      ...
      const partes = ["formularios_web"];
      if (filtros.fuente) partes.push(filtros.fuente.replace(/[^\w-]+/g, "-").toLowerCase());
