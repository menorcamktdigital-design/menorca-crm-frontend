...
/* ---------- Cache en PostgreSQL ------------------------------------------ */
const TABLA_CACHE = "ventas_historico_cache";
let tablaCreada = false;

async function asegurarTabla() {
  if (tablaCreada) return;
  await query(`
    CREATE TABLE IF NOT EXISTS ${TABLA_CACHE} (
      mes INT PRIMARY KEY,
      data JSONB NOT NULL,
...
async function getCached(mes: number) {
  await asegurarTabla();
  const rows = await query<{ data: unknown; created_at: Date }>(
    `SELECT data, created_at FROM ${TABLA_CACHE} WHERE mes = $1`,
    [mes]
  );
  if (rows.length === 0) return null;

  const mesActual = new Date().getMonth() + 1;
  if (mes < mesActual) return rows[0].data;

  const age = Date.now() - new Date(rows[0].created_at).getTime();
  if (age < 5 * 60_000) return rows[0].data;

  return null;
}

async function setCache(mes: number, data: unknown) {
  await asegurarTabla();
  await query(
    `INSERT INTO ${TABLA_CACHE} (mes, data, created_at)
     VALUES ($1, $2, NOW())
...
export async function GET(req: NextRequest) {
  ...
  try {
    const cached = await getCached(mesNum);
    if (cached) {
      return NextResponse.json(cached);
    }
  } catch {
    // Si falla la BD, seguimos sin cache
  }
  ...
  const resultado = { mes: mesNum, total: ventas.length, por_canal, ventas };

  try {
    await setCache(mesNum, resultado);
  } catch {
    // Si falla guardar en BD, no es crítico
  }
